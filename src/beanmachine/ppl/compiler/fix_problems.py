...
def arithmetic_graph_fixer(skip: Set[str]) -> GraphFixer:
    def arith_graph_fixer(bmg:BMGraphBuilder) -> GraphFixerResult:
        typer = LatticeTyper()
        node_fixers = [
...
def fix_problems(
    ...
    all_fixers: Callable[[BMGraphBuilder], tuple[BMGraphBuilder, bool, ErrorReport]] = sequential_graph_fixer(
        [
            ...
            requirements_fixer(),
            observations_fixer(),
            conditional_graph_fixer(bmg._fix_observe_true, observe_true_fixer()),
        ]
    )
    current, _, errors = all_fixers(bmg)
