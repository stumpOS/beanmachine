...
def sequential_graph_fixer(fixers: List[GraphFixer]) -> GraphFixer:
    ...
    def sequential(initial_builder: BMGraphBuilder) -> GraphFixerResult:
        made_progress = False
        errors = ErrorReport()
        current_builder = initial_builder
        for fixer in fixers:
            current_builder, fixer_made_progress, errors = fixer(current_builder)
            made_progress |= fixer_made_progress
            if errors.any():
                break
        return current_builder, made_progress, errors
