...
import beanmachine.ppl.compiler.paic.mlir.test_paic_mlir.infer.meta_ad
...
def test_infer_python():
    ...
    # now try with Sliced version, Python
    sliced_nnc = Sliced_NMC(ad= beanmachine.ppl.compiler.paic.mlir.test_paic_mlir.infer.meta_ad.PyTorchAD(), lower = False)
    sliced_samples = sliced_nnc.infer(queries=[model.foo()], observations=observations, num_samples=100)
    sliced_foo_samples = sliced_samples.get_variable(model.foo())
    sliced_approx_foo = torch.mean(sliced_foo_samples)
    assert abs(sliced_approx_foo - foo_value) < 1.0

    # now try with Sliced version, MLIR
    sliced_opt_nnc = Sliced_NMC(ad= beanmachine.ppl.compiler.paic.mlir.test_paic_mlir.infer.meta_ad.PyTorchAD(), lower = True)
    sliced_opt_samples = sliced_opt_nnc.infer(queries=[model.foo()], observations=observations, num_samples=100)
    sliced_opt_foo_samples = sliced_opt_samples.get_variable(model.foo())
    sliced_opt_approx_foo = torch.mean(sliced_opt_foo_samples)
    assert abs(sliced_opt_approx_foo - foo_value) < 1.0
